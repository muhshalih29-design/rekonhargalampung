...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_commodity') {
    ...
    if ($commodity_name === '') {
        header('Location: hkd.php?notice_type=error&notice=' . urlencode('Komoditas yang akan dihapus belum dipilih.'));
        exit;
    }
    if (!in_array($delete_scope, ['current', 'all'], true)) {
        header('Location: hkd.php?notice_type=error&notice=' . urlencode('Pilihan hapus komoditas tidak valid.'));
        exit;
    }
    ...
    if ($delete_scope === 'current') {
        if ($filter_bulan === '' || $filter_tahun === '' || !ctype_digit($filter_tahun)) {
            ...
            header('Location: hkd.php?' . http_build_query($redirect_query));
            exit;
        }
        $delete_stmt = $pdo->prepare('DELETE FROM hkd WHERE TRIM(LOWER(komoditas)) = TRIM(LOWER(?)) AND TRIM(LOWER(bulan)) = ? AND tahun = ?');
        ...
        $delete_stmt = $pdo->prepare('DELETE FROM hkd WHERE TRIM(LOWER(komoditas)) = TRIM(LOWER(?))');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_commodity') {
    ...
    $commodity_name = preg_replace('/\s+/', ' ', trim($commodity_raw));
    if ($commodity_name === '') {
        header('Location: hkd.php?notice_type=error&notice=' . urlencode('Nama komoditas tidak boleh kosong.'));
        exit;
    }
    ...
    $stmt_kab = $pdo->query("SELECT DISTINCT kode_kabupaten, nama_kabupaten FROM hkd WHERE TRIM(COALESCE(kode_kabupaten, '')) <> '' AND TRIM(COALESCE(nama_kabupaten, '')) <> '' ORDER BY CAST(kode_kabupaten AS INTEGER) ASC, nama_kabupaten ASC");
    $kabupaten_rows = $stmt_kab->fetchAll();
    if (empty($kabupaten_rows) || empty($months_to_create)) {
        header('Location: hkd.php?notice_type=error&notice=' . urlencode('Data kabupaten/kota dasar belum tersedia.'));
        exit;
    }

    $insert_stmt = $pdo->prepare("
        INSERT INTO hkd (kode_kabupaten, nama_kabupaten, bulan, tahun, komoditas, time_stamp)
        SELECT ?, ?, ?, ?, ?, NOW()
        WHERE NOT EXISTS (
            SELECT 1
            FROM hkd
    ...
    try {
        ...
        header('Location: hkd.php?notice_type=error&notice=' . urlencode('Gagal menambahkan komoditas baru.'));
        exit;
    }
    ...
    header('Location: hkd.php?bulan=' . urlencode($current_month) . '&tahun=' . $current_year . '&komoditas=' . urlencode(strtolower($commodity_name)) . '&notice_type=' . urlencode($notice_kind) . '&notice=' . urlencode($notice_text));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update') {
    ...
    if (is_kabupaten($user)) {
        $stmt_kab = $pdo->prepare('SELECT kode_kabupaten FROM hkd WHERE id = ?');
        $stmt_kab->execute([$id]);
    ...
    if ($type === 'decimal') {
        ...
        if ($raw === '') {
            $sql = "UPDATE hkd SET {$field} = NULL WHERE id = ?";
            ...
            $sql = "UPDATE hkd SET {$field} = ? WHERE id = ?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$num, $id]);
        }
    } elseif ($type === 'enum') {
...
foreach ($stmt_pending as $pending_row) {
    ...
    if ($k === '') continue;
    $komoditas_pending_map[strtolower($k)] = [
        'label' => $k,
        'count' => (int)($pending_row['pending_count'] ?? 0),
    ];
}
$pending_items = [];
foreach ($komoditas_tabs as $item) {
    $key = $item['key'];
    $count = (int)(($komoditas_pending_map[$key]['count'] ?? 0));
    if ($count > 0) {
        $pending_items[] = ['label' => $item['label'], 'count' => $count];
    }
}
  ...
  <body>
    <div class="app">
      <aside class="sidebar">
        ...
        <a class="nav-dot active" href="hkd.php" title="HKD"><span class="nav-text">HKD</span></a>
      ...
      <main class="main">
        <form class="topbar" method="get" action="hkd.php">
          <div>
            <div class="hello">Konfirmasi Perubahan Harga Konsumen Pedesaan</div>
        ...
        <?php if (is_provinsi($user)): ?>
          <div class="commodity-add-card">
            <div class="commodity-add-copy">
              ...
              <div>
                <div class="commodity-add-title">Tambah Komoditas HKD</div>
            ...
            <form class="commodity-add-form" method="post" action="hkd.php">
              <input type="hidden" name="action" value="add_commodity">
              <input type="text" name="commodity_name" class="commodity-add-input" placeholder="Contoh: Telur Itik" maxlength="100" required>
          ...
          <div class="delete-modal" id="delete-commodity-modal" aria-hidden="true">
            <div class="delete-panel" role="dialog" aria-modal="true" aria-labelledby="delete-commodity-title">
              <h3 id="delete-commodity-title">Hapus Komoditas HKD</h3>
              ...
              <form method="post" action="hkd.php" id="delete-commodity-form">
                ...
                <div class="delete-scope">
                  ...
                  <label class="delete-option" id="delete-option-all">
                    ...
                    <div>
                      ...
                      <span>Menghapus seluruh data komoditas dari Januari sampai Desember 2026 yang tersedia di tabel HKD.</span>
                    </div>
                  </label>
                </div>
        ...
        <?php if (!empty($komoditas_tabs)): ?>
          <div class="tabs" data-selected="<?php echo htmlspecialchars($komoditas_selected); ?>">
            <?php foreach ($komoditas_tabs as $item): ?>
              ...
              <?php $is_done = ((int)(($komoditas_pending_map[$item['key']]['count'] ?? 0)) === 0) ? 'is-done' : ''; ?>
              <button type="button" class="tab-btn <?php echo trim($is_active . ' ' . $is_done); ?>" data-komoditas="<?php echo htmlspecialchars($item['key']); ?>">
